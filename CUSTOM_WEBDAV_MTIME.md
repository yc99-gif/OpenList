...
当前实现已经能够在不依赖上游协议能力的情况下，为所有经过 OpenList WebDAV 层的存储驱动提供虚拟原始时间、明文校验值和自定义属性。Google Drive、S3/R2 等存储首先应使用现有版本做真实 WebDAV 兼容性测试；只要客户端重复同步、条件请求和时间比较正常，就不需要为了当前可用性修改驱动。

如果以后需要在 OpenList 数据目录丢失、多实例访问或绕过 OpenList 直接读取上游时仍能恢复部分元数据，可以增加第二层、可选的“上游原生元数据同步”。建议保留当前 `webdav_metadata` 表作为 WebDAV 权威和快速数据源，同时建立统一的内部元数据载体与驱动能力声明，而不是在 WebDAV Handler 中按驱动名称写分支。内部载体至少要区分：客户端是否显式提供修改/创建时间、时间精度、可信校验值、普通 dead property，以及这些值是否允许离开本机。

各驱动只负责映射自己原生支持的部分：

- Google Drive：可研究把显式修改时间映射到 `modifiedTime`，继续读取 Google 返回的 MD5、SHA-1、SHA-256，并在确有必要时使用 `appProperties` 保存 OpenList 专用值。当前驱动已经读取时间和校验值，但上传请求尚未使用 `FileStreamer` 中的原始时间。
- S3 / Cloudflare R2：原生 `Last-Modified` 仍代表对象写入时间，原始文件时间可选地保存为 `x-amz-meta-openlist-mtime` 等自定义元数据；OpenList 逻辑 ETag 不应强行替换对象存储 ETag。标准目录列表通常不能批量返回全部自定义元数据，逐对象 `HeadObject` 会增加延迟、请求量和费用，因此本地数据库仍应作为主要读取路径，上游元数据更适合作为恢复辅助。
- Crypt：禁止默认把明文 SHA-256、原始时间或 dead property 原样写入上游。明文哈希会暴露内容相等性，时间也可能泄漏活动信息；如需上游持久化，应提供“不传、选择性传递、加密元数据或加密 sidecar”等明确策略。
- 不支持原生元数据的驱动：无需报错或单独重做 WebDAV，继续使用本地 sidecar 回退。

时间在上传开始前已知，比较适合直接传给有能力的驱动；OpenList 自行计算的明文 SHA-256 只有在流式上传完成后才确定。若上游要求在开始上传时提交校验值，就需要预读、临时缓存、上传后更新元数据，或者仍只保存在本地数据库。任何实现都应先评估大文件额外 I/O、对象存储请求成本、分片上传语义和 Crypt 隐私影响。

推荐的实施顺序是：先分别对 Google Drive、R2 以及它们外层的 Crypt 挂载执行 PUT、PROPFIND、条件 GET/PUT、重启和五分钟稳定期测试；只有测试或恢复需求证明本地 sidecar 不足时，再增加通用元数据载体、驱动能力接口和单个驱动的参数映射。
